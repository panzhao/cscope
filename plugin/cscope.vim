  ...
  while cwd != ''
    ...
    for fn in a
      ...
      elseif fn =~ g:cscope_ignore_files
        continue
      elseif !(fn =~ g:cscope_enable_files)
        continue
      else
        if stridx(fn, ' ') != -1
          let fn = '"'.fn.'"'
...
function! s:_CreateCscopeDB(dir,id)
  call system('$HOME/.vim/plugin/generate_lookupfile_tags.sh')
  if ! isdirectory(s:cscope_vim_dir)
    call mkdir(s:cscope_vim_dir)
  endif
  ...
  exe '!'.g:cscope_cmd.' -b -i '.cscope_files.' -v '
